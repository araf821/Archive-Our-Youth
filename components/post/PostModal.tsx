...
const PostModal = ({ children, post, currentUser }: PostModalProps) => {
      ...
      <DialogContent className="w-full max-w-screen-sm rounded-sm border border-zinc-700 bg-[#202020] p-4 text-zinc-100 shadow-[0_0_20px_5px_black] outline-none max-md:w-[95%] md:rounded-md md:p-6">
        <div className="flex items-center justify-between">
          ...
          <div className="relative">
            ...
            <Link
              href={`/post/${post.slug}`}
              className="relative z-10 flex rounded-md bg-green-500 px-2 py-1 text-center font-medium tracking-wide text-zinc-950 transition duration-200 hover:-translate-x-0.5 hover:-translate-y-0.5 hover:bg-green-600 hover:text-white active:translate-x-1 active:translate-y-1"
            >
              View Post
            </Link>
          </div>
