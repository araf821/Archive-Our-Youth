...
const AudioItem: FC<AudioItemProps> = ({ onClick, post, clicked, onClose }) => {
  ...
  return (
    <button
      ...
      className="group relative grid aspect-square h-full w-full cursor-pointer place-items-center overflow-hidden border border-border-dark bg-zinc-900 outline-none focus-visible:z-[9999] focus-visible:outline focus-visible:outline-4 focus-visible:outline-white"
    >
      ...
      <div className="absolute left-0 top-0 z-20 rounded-br-md bg-black/75 p-1.5 text-zinc-100 max-sm:text-xs sm:text-sm">
        <Volume2 className="size-4 md:h-5 md:w-5" />
      </div>
      {!post.thumbnail && (
        <>
